...
function getDoubleHelixPositions(radius = 300, height = 600) {
  const positions = [];
  const half = Math.floor(count / 2);

  for (let i = 0; i < half; i++) {
    const t = i / half;
    const angle = t * Math.PI * 2 * 5; // 5 full turns
    positions.push({
  ...
  for (let i = 0; i < count - half; i++) {
    const t = i / (count - half);
    const angle = (t * Math.PI * 2 * 5) + Math.PI; // offset by 180°
    positions.push({
      x: Math.cos(angle) * radius,
      y: (t * height) - height / 2,
      z: Math.sin(angle) * radius
...
function captureWork() {
  const data = getDoubleHelixPositions();
